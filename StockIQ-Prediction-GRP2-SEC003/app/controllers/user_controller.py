from flask import jsonify, request, render_template, redirect, session, current_app, flash
...
def home():
    ...
    # Get balance for logged-in user to show on dashboard
    user = mongo.db.users.find_one({"_id": ObjectId(session['user_id'])})
    balance = float(user.get("amount", 0)) if user else 0.0
...
def buy_stock():
    ...
    if data.empty:
        flash("Unable to fetch current price for this stock.", "error")
    ...
    if total_cost > balance:
        # ✅ Show error banner instead of JSON
        flash("Insufficient funds to complete this purchase.", "error")
        return redirect('/dashboard')
...
def add_funds():
    ...
    current_balance = float(user.get("amount", 0))
...
def login_user():
    if request.method == 'POST':
        ...
        if user:
            ...
            # set the display name for navbar
